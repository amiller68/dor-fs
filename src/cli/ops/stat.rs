...
use crate::cli::config::{Config, ConfigError, working_dot_dir, DEFAULT_LOCAL_DOT_CHANGELOG};
...
pub fn stat(_config: &Config, working_dir: PathBuf) -> Result<DisaplyableChangelog, StatError> {
    ...
    let changelog = match std::fs::read_to_string(working_dot_dir_path.join(DEFAULT_LOCAL_DOT_CHANGELOG)) {
        Ok(s) => {
